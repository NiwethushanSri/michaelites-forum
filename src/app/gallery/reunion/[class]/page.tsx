...
const classData: Record<string, { label: string; photos: string[] }> = {
  "class-a": {
    label: "Class A",
    photos: ["/reunion-2020.jpg"],
  },
  "class-b": { label: "Class B", photos: [] },
  "class-c": { label: "Class C", photos: [] },
  "class-d": { label: "Class D", photos: [] },
  "class-e": { label: "Class E", photos: [] },
};
    ...
    <>
      ...
      <div className="bg-gradient-to-r from-[#0d1b3e] to-[#1a2d5a] text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <Link href="/gallery/reunion" className="inline-flex items-center gap-2 text-gray-400 hover:text-white text-sm mb-6 transition-colors">
            <ChevronLeft className="w-4 h-4" /> Back to Reunion
          </Link>
          <h1 className="text-3xl md:text-4xl font-black mb-1">
            Reunion · <span className="text-[#c9a227]">{data.label}</span>
          </h1>
          <p className="text-gray-400 text-sm">{data.photos.length} photo{data.photos.length !== 1 ? "s" : ""} · Michaelites A/L 2015</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="bg-[#0d1b3e] text-white rounded-2xl p-5 mb-10 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-bold text-sm">Have photos from Reunion – {data.label}?</p>
        ...
        {data.photos.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.photos.map((src, i) => (
              <div key={i} className="rounded-2xl overflow-hidden shadow-md border border-gray-100 aspect-video relative group">
                <Image
                  src={src}
                  alt={`Reunion ${data.label} – photo ${i + 1}`}
                  fill
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h3 className="text-xl font-bold text-[#0d1b3e] mb-2">No photos yet for {data.label}</h3>
            <p className="text-gray-500 text-sm max-w-sm mb-6">
              Be the first to upload reunion memories for {data.label}, or sign in to view private photos.
            </p>
